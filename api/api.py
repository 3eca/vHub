...
async def video_share():
    ...
    sh_video = share_video(data['videoLink'], data['state'])
    if sh_video:
        LOGER.info(
        ...
        LOGER.info(
            f'{video_share.__name__}(): user "{current_user.name}" try sharred="{data["state"]}" video "{data["videoName"]}" with link "{data["videoLink"].split(".")[0]}". '
            f'Option disable. '
            f'Remote addr: "{request.headers["X-Forwarded-For"]}".'
            )
        return jsonify(status=False, message='Option disable.')
    LOGER.error(
